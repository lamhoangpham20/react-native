...
export default class AuthDemo extends Component {
  ...
  componentDidMount() {
    ...
    SecureStore.getItemAsync('demoApplicationJWT10')
      .then(response => {
        ...
        fetch(this.props.apiURI + '/jwt/jwtProtectedResource', {
          ...
          this.props.userLogin(json.user);
        }).catch(error => {
  ...
  onLoginReceiveJWT = (responseJWT) => {
    ...
    SecureStore.setItemAsync('demoApplicationJWT10', responseJWT)
      .then(response => {
  ...
  authLogic = () => {
    ...
    const app = (
      <Stack.Screen
        ...
        >
          { props => <User {...props} user={ this.state.user }></User>}
        </Stack.Screen>
    )
